...
**HTTP surfaces registered in** `main.py`: the module-owned My Job, Learning, Memory, To-do, Home, Google Calendar/Gmail, Attachments, Images, and Speech APIs plus the legacy health, legal, auth, admin, webhooks, users, link_preview, chats, chat_stream, models, search, suggestions, analytics, and ws routers.
...
- Network: `lib/api.ts` barrel → `lib/api/{client,auth,chats,account,discover,connectivity,analytics,push,types}.ts` plus feature slices (`features/learning/api.ts`, `features/memory/api.ts`, `features/todos/api.ts`, `features/home/api.ts`, `features/job-search/api.ts`, `features/integrations/api.ts`, `features/attachments/api.ts`, `features/images/api.ts`, `features/speech/api.ts`)
