...
[`math/tools/`](../apps/api/app/services/math/tools/) is the feature split: ordered `_INTENT_EXTRACTORS` in `extract.py` plus `kind → _verified_block_*` in `block/`. Do **not** add a second kind table. Rendering: inline `MathText`, display math MathJax-SVG (`MathSvgView`), geometry diagrams `react-native-svg`, and native math graph surfaces Skia (`rich/skia/`, with SVG fallback).
