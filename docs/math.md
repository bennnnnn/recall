...
| Arithmetic / percent / ratio | Bare `7*8` / `8-8*2`; `15% of 80`; increase/decrease by `%`; `what percent of`; simplify `6:8`; split a total in `a:b`; AP/GP nth term and first-N sum (listed or first N even/odd); annual simple/compound interest; union/intersection/difference of two `{…}` sets; work-together (two hour-times); two-part `%` mixture; `twice as many` + together + one total | `arithmetic` |
...
| Linear algebra | 2×2–4×4 det, inverse, multiply, rref, eigenvalues (`[[…]]` bracket notation only) | `matrix` |
| Calc II (thin) | Taylor / Maclaurin, partials, first-order `dsolve`, 2nd/3rd derivative, double integrals over named `x`/`y` rectangles. Written proofs stay LLM | `calculus` |
...
Still not a verified kind (the model may answer; it must **not** claim a verified result). Percent change, percent-is, ratio split, AP/GP sequences, annual simple/compound interest, two-set union/intersection/difference, the narrow work-together / two-part mixture / twice-as-many templates, identities that simplify to 0, polar/parametric graphs, named-axis double integrals, and matrix multiply/rref/eigenvalues are verified on existing kinds.

1. **Open-ended word problems** (age puzzles, leftover quantities, stories that are not one of the three templates) stay LLM-only.
...
4. **Physics beyond the verified templates** — friction, tension, normal-force systems, momentum/collisions, rotation, circuits, waves, thermodynamics, relativity, coupled ODEs, and free-body diagrams remain LLM-only.
