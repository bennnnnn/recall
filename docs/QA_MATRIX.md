...
| 5.7 | Automations — free user create attempt shows Upgrade sheet | ☐ | ☐ | No network call; client-side gate |
| 5.8 | Automations — Pro create (prompt + frequency + time), appears in list | ☐ | ☐ | |
| 5.9 | Automations — active cap (`automations_max_active_per_user`) shows inline error, not a crash | ☐ | ☐ | Create N+1 |
| 5.10 | Automations — detail transcript renders after a scheduled run (web search only, no calendar/email/image tool calls) | ☐ | ☐ | Needs worker running; verify via `last_run_status` |
| 5.11 | Automations — pause/resume via detail overflow menu | ☐ | ☐ | Paused automation stops appearing in scheduler `list_due` |
| 5.12 | Automations — edit prompt/frequency/time | ☐ | ☐ | |
| 5.13 | Automations — delete removes automation + its chat (drawer never shows it) | ☐ | ☐ | |
...
| 7.4 | Automation-run push notification tap opens `/automations/{id}` | ☐ | ☐ | Dev build; worker must complete a run |
