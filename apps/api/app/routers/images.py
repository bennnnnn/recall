...
from app.services.attachment_content import content_type_for_image_bytes, is_image_content_type
...
@router.get("/usage/today", response_model=ImageGenerationUsageOut)
async def today_image_generation_usage(
    user: User = Depends(get_current_user),
    settings: Settings = Depends(get_settings_dep),
) -> ImageGenerationUsageOut:
    redis = get_redis_client()
    limit = quota_service.image_generation_limit_for_user(user, settings)
    used = await quota_service.get_image_generation_used(redis, user.id)
    return ImageGenerationUsageOut(
        used=used,
        daily_limit=limit,
...
@router.post("/usage/today/reset", response_model=ImageGenerationUsageOut)
async def reset_today_image_generation_usage(
    user: User = Depends(get_current_user),
    settings: Settings = Depends(get_settings_dep),
) -> ImageGenerationUsageOut:
    ...
    redis = get_redis_client()
    await quota_service.reset_image_generation_usage(redis, user.id)
    return await today_image_generation_usage(user, settings)
...
async def generate_image(
    ...
    if await quota_service.is_image_generation_exhausted(redis, user.id, limit=daily_limit):
        raise HTTPException(
    ...
    image_bytes, claimed_type = generated
    content_type = content_type_for_image_bytes(image_bytes, claimed=claimed_type)
    if not content_type or not is_image_content_type(content_type):
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
