"""Materialize To-do mutations from assistant ``reminder`` JSON fences."""
...
_INVALID_FENCE = "*Could not save that to-do — the format was invalid.*"
_WEEKDAYS = (
    "monday",
    "tuesday",
    "wednesday",
...
_FAIL_VERB = {
    ...
    "uncheck": "reopen",
    "set_due": "reschedule",
    "clear_due": "remove the date from",
}
_OK_PREFIX = {
    "add": "Set",
    "delete": "Deleted",
...
class _ReminderFence(BaseModel):
    ...
    action: Literal["add", "complete", "uncheck", "delete", "set_due", "clear_due"] = "add"
    title: str = Field(
        min_length=1,
        max_length=500,
    ...
    def validate_date_and_repeat(self) -> Self:
        if self.action == "set_due" and self.due_at is None:
            raise ValueError("set_due requires due_at")
        if self.repeat is not None and self.due_at is None:
            raise ValueError("repeat requires due_at")
        return self
...
async def _load_existing(state: _ReminderFenceCreateState) -> None:
    """Load To-dos once (lazily) so multi-fence replies don't re-read the DB."""
...
def _existing_mutation_match(
    state: _ReminderFenceCreateState, title: str, action: str
) -> TodoItem | None:
    needle = title.lower()
    matches = [item for item in state.existing if (item.content or "").strip().lower() == needle]
    if action == "uncheck":
        return next((item for item in matches if item.checked), matches[0] if matches else None)
    return next((item for item in matches if not item.checked), matches[0] if matches else None)
...
async def _create_one(state: _ReminderFenceCreateState, draft: _ReminderFence) -> tuple[str, bool]:
    ...
    try:
        new_todo = await todos_repo.create(
            ...
            content=title,
            topic=REMINDER_TOPIC if due_at is not None else todos_repo.DEFAULT_TOPIC,
    ...
    logger.info(
        "To-do fence applied: user_id=%s chat_id=%s title=%s",
        state.user_id,
        state.chat_id,
        title[:80],
    )
...
async def _mutate_one(state: _ReminderFenceCreateState, draft: _ReminderFence) -> tuple[str, bool]:
    ...
    due_at = time_context_service.normalize_due_at(draft.due_at, state.user_timezone)
    await _load_existing(state)
    existing = _existing_mutation_match(state, title, draft.action)
    action = TodoActionItem(
        action=draft.action,
        topic=existing.topic if existing is not None else REMINDER_TOPIC,
        content=title,
        due_at=draft.due_at,
...
async def materialize_reminder_fences(
    ...
    *,
    ...
    """Apply To-do fences (create + mutations) and strip them from the reply.
