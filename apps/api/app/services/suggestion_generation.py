"""Generate proactive suggestions from recent user context."""
...
async def _load_suggestion_snapshot(
    ...
    recent_summary = "\n".join(
        f"- {chat.title or 'New chat'} (updated {chat.updated_at.strftime('%b %d')})"
        for chat in recent
    )
    ...
    return _SuggestionSnapshot(
        user_context=user_context,
        max_items=min(3, MAX_ACTIVE_SUGGESTIONS - active_count),
    )
...
async def _apply_suggestion_result(
    ...
    existing_texts = {suggestion.text.strip().lower() for suggestion in existing if suggestion.text}
...
async def generate_suggestions(settings: Settings, user_id: UUID) -> None:
    try:
