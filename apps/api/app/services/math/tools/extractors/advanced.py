...
_BOUND = r"[-+]?(?:\d+(?:\.\d+)?|\.\d+|pi|π|e)(?:\s*/\s*(?:\d+(?:\.\d+)?|pi|π|e))?"
_FROM_TO_RE = re.compile(
    rf"\bfrom\s+(?:x\s*=\s*)?({_BOUND})\s+to\s+(?:x\s*=\s*)?({_BOUND})\b",
    re.IGNORECASE,
)
_ON_INTERVAL_RE = re.compile(rf"\bon\s*\[\s*({_BOUND})\s*,\s*({_BOUND})\s*\]", re.IGNORECASE)
...
def _bounds(text: str) -> tuple[str, str, int] | None:
    ...
    if match is None:
        return None
    return match.group(1).replace("π", "pi"), match.group(2).replace("π", "pi"), match.start()
...
def _extract_advanced_matrix_intent(cleaned: str) -> MathIntent | None:
    ...
    elif "nullity" in lower or "dimension of the null space" in lower or "dimension of null space" in lower:
        op = "matrix_nullity"
...
def _extract_bivariate_statistics_intent(cleaned: str) -> MathIntent | None:
    ...
    if "correlation" in lower or "pearson" in lower:
        ...
        op = "population_covariance" if "population" in lower else "sample_covariance"
    elif any(
        cue in lower
        for cue in (
            "linear regression",
...
def _extract_function_composition(cleaned: str) -> MathIntent | None:
    ...
    # Explicit named definitions only.  Requiring both assignments prevents
    # us from silently composing a guessed function from surrounding prose.
    patterns = (
        re.compile(
    ...
    op = "function_compose_fg" if "f(g(x))" in compact else "function_compose_gf"
    return MathIntent(
        kind="calculus",
        school_op=op,
        expr=f_expr,
...
def _extract_function_intent(cleaned: str) -> MathIntent | None:
    ...
    for cue, op in (
        ...
        if op == "function_inverse" and "[[" in cleaned:
            return None
        if op == "function_inverse" and "inverse of" == cue and "f(" not in lower[idx:]:
    ...
    if "even or odd" in lower or "odd or even" in lower:
        match = re.search(
            r"(?:f\s*\(\s*x\s*\)|y)\s*=\s*(.+?)(?=\s+(?:even|odd)\b|[?.!]*$)",
            cleaned,
            re.IGNORECASE,
        )
        if match is None:
...
def _extract_calculus_application_intent(cleaned: str) -> MathIntent | None:
    ...
    if "volume" in lower and any(word in lower for word in ("revolution", "revolved", "rotated")):
        ...
        if axis is None:
            return None
        # Accept the most common school phrasing: "volume of revolution of
        # y=f(x) from a to b about the x-axis".  Everything after the bounds is
        # deliberately ignored only after the axis has been recognized.
