...
MATH_SOLVER_HINT = (
    ...
    "waves, optics, thermal, gravitation, fluids, rotation. Use a verified physics "
    "block's exact numbers — never "
    "recompute. Include units (g = 9.81 m/s^2 on Earth). When working is requested, "
    "give the general equation then substitute; state the result once in `$...$`. Do "
    "NOT re-list sampled points in prose, and do NOT emit ```graph or ```simulation. "
    ...
    "other answer is a number — do not invent a plot. OUTSIDE that list (magnetism, "
    "materials, relativity, nuclear, alternating current) nothing is checked: do NOT claim "
    "verification; be cautious and say when you are unsure."
)
