...
MATH_INTENT_HINT = (
    ...
    "system block is present, Recall attaches a diagram after your answer, and "
    "attaches a verified result pill only when the value is not already in your "
    "prose. Closed-form and one-line arithmetic: the `$...$` line is the answer — "
    "do not restate it. Multi-step: show steps in `$...$` and do not add a boxed "
    ...
    "  - Numbered `$...$` lines for homework — not a step-card fence.\n"
...
MATH_SOLVER_HINT = (
    ...
    "Recall attaches a verified result only when it is not already in your prose. "
    ...
    "values. Recall attaches a trajectory graph; state the numeric result once "
...
SHORT_MATH_SAFETY_HINT = (
    ...
    "Do NOT emit ```answer, ```graph, or ```geometry — Recall attaches verified "
    "diagrams, and a result pill only when the value is not already in your prose. "
