...
MATH_INTENT_HINT = (
    ...
    "`- For F = 0:`\n"
    "`  $0 + 3 = 3$`). Wrap that second line in `$...$`. Every For-line MUST "
    "end with a colon (`- For x = 3:` and `- For x = 1/2:`) — never omit the "
    "colon on one root. If the substitution has more than one `=`, put EACH "
    "equality on its own `$...$` line and end with the residual (`$= 0$`) — "
    "never one long `$a = b = c = 0$` (clips). "
    "End with `- [x]` or a trailing check mark.\n"
    ...
    "lines. Do not stop at `$\\frac{7 \\pm 5}{4}$` or jump to `$x = 3$ or $x = 1/2$`. "
    "Join two roots with the word or or a comma only in the final statement "
...
SHORT_MATH_SAFETY_HINT = (
    ...
    "diagram. Do not add a boxed final-answer section when the value is already "
    "in your prose. "
