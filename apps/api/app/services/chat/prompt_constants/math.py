...
MATH_INTENT_HINT = (
    ...
    "For a worked solution, one mathematical transformation per numbered step: "
    "short operation label, then its formula alone on the next `$...$` line, "
    "with a blank line before the next step. Copy verified formulas; do not "
    "recompute them or combine subtract-and-divide into one step.\n"
    ...
    "with the formula. Inverse operations are for equations, not a universal "
    "teaching method.\n"
...
MATH_SOLVER_HINT = (
    ...
    "- Limits, series, statistics, combinatorics, number theory, and small matrices: "
    "ONLY when a verified math block is present—use its exact result and do NOT "
    "recompute. Without one, do NOT claim verification; be cautious and say when "
    "you are unsure.\n"
    "- Physics — verified: kinematics, SUVAT (constant acceleration), projectile, "
    "force, energy, momentum, friction, circular, spring, pendulum, circuit, torque, "
    "waves, optics, thermal, gravitation, fluids, rotation, magnetism, materials, "
    "modern, relativity, quantum, interference/diffraction, heat transfer, and fluid "
    "transport. Use a verified physics block's exact numbers — never "
    "recompute. Include units. Verified physics must separate **Given**, "
    "**Find**, **Formula**, **Substitution**, **Answer** in that order, one equation per "
    "line and one final result. Never re-list sampled "
    "points, or emit ```graph or ```simulation. "
    "Trajectory graphs exist only for kinematics/SUVAT (height or velocity against "
    "time), "
    "projectile (x-y path) and spring/pendulum SHM (displacement against time); every "
    "other answer is a number — do not invent a plot. Outside that list (quantum states, "
    "alternating current, Maxwell boundary problems, Lagrangian mechanics, Hamiltonian mechanics), "
    "do NOT claim verification; be cautious and say when you are unsure. For advanced "
    "derivations, put the governing equation first; finish in at most 4 short steps "
    "and 350 words, without a textbook survey."
)
