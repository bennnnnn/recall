...
def test_ensure_project_daily_quiz():
    user = _fake_user()
    app = _app_with_user(user)
    project = _project(user_id=user.id, kind="language")

    from datetime import date

    from app.models.schemas import ProjectDailyQuizOut

    payload = ProjectDailyQuizOut(
    ...
    with (
        ...
        patch(
            "app.routers.projects.daily_quiz_service.ensure_daily_quiz",
            AsyncMock(return_value=payload),
        ),
        patch(
            "app.routers.projects.home_service.invalidate_home_cache",
            AsyncMock(),
        ),
    ):
        client = TestClient(app)
        r = client.post(
            f"/projects/{project.id}/quiz/daily/ensure",
            headers={"Authorization": "Bearer tok"},
        )

    assert r.status_code == 200
    assert r.json()["daily_goal"] == 5
