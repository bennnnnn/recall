...
def test_list_templates():
    ...
    tpl_mock.content = "Draft an email about..."
    tpl_mock.category = "writing"
    tpl_mock.is_builtin = True
    tpl_mock.created_at = now
    tpl_mock.updated_at = now

    user = _fake_user()
    app = _app_with_user(user)
    with patch(
        "app.routers.templates.templates_repo.list_for_user",
        AsyncMock(return_value=[tpl_mock]),
    ):
        client = TestClient(app)
        r = client.get("/templates", headers={"Authorization": "Bearer tok"})
    assert r.status_code == 200
    data = r.json()
    assert len(data) == 1
    assert data[0]["title"] == "Write email"
...
def test_create_template():
    ...
    tpl_mock.content = "Do the thing"
    tpl_mock.category = "general"
    tpl_mock.is_builtin = False
    tpl_mock.created_at = now
    tpl_mock.updated_at = now
    ...
    app = _app_with_user(user)
    with patch(
        "app.routers.templates.templates_repo.create",
        AsyncMock(return_value=tpl_mock),
    ):
        client = TestClient(app)
        r = client.post(
            "/templates",
            headers={"Authorization": "Bearer tok"},
            json={"title": "Custom prompt", "content": "Do the thing"},
        )
    assert r.status_code == 201
    assert r.json()["title"] == "Custom prompt"


def test_update_template_owned():
    from fastapi.testclient import TestClient
    ...
    tpl_mock.is_builtin = False
    tpl_mock.title = "Updated"
    tpl_mock.content = "New content"
    tpl_mock.category = "general"
    tpl_mock.created_at = now
    tpl_mock.updated_at = now
    ...
    with (
        patch(
            "app.routers.templates.templates_repo.get_by_id",
            AsyncMock(return_value=tpl_mock),
        ),
        patch(
            "app.routers.templates.templates_repo.update",
            AsyncMock(return_value=tpl_mock),
        ),
    ):
        client = TestClient(app)
...
def test_update_template_builtin_denied():
    """Built-in templates cannot be edited by any user."""
    from fastapi.testclient import TestClient
...
def test_update_template_wrong_owner_denied():
    """User B cannot edit user A's template — get_by_id now scopes by user."""
    from fastapi.testclient import TestClient

    tid = uuid4()
    other_user = _fake_user(id=uuid4())
    app = _app_with_user(other_user)
    # Repository returns None when the user doesn't own the template.
    with patch(
        "app.routers.templates.templates_repo.get_by_id",
...
def test_delete_builtin_denied():
    from fastapi.testclient import TestClient

    user = _fake_user()
    app = _app_with_user(user)
    with patch(
        "app.routers.templates.templates_repo.delete_by_id",
        AsyncMock(return_value=False),  # repo also guards is_builtin
    ):
        client = TestClient(app)
        r = client.delete(
            f"/templates/{uuid4()}",
...
async def test_seed_templates_creates_on_empty():
    ...
    # Simulate empty DB: no built-in templates exist yet.
    ...
    # Should add all built-in templates.
...
async def test_seed_templates_idempotent():
    ...
    # Simulate already-seeded DB.
    existing = MagicMock(spec=Template)
    mock_result = MagicMock()
