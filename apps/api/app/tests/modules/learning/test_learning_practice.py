...
from app.modules.learning.crud import LearningError
from app.modules.learning.items_repository import count_stats_sql
from app.modules.learning.practice import record_practice
from app.modules.learning.schemas import LearningPracticeIn
...
def practice_sql():
    ...
    try:
        with Session(engine, expire_on_commit=False) as real:
            ...
            real.commit()
            with patch("app.modules.learning.practice._invalidate_home_for_user", AsyncMock()):
...
async def test_sql_stats_count_reviewed_words_without_counting_them_as_new(practice_sql):
    ...
    await record(practice_sql, outcome(complete=True))
    with patch(
        "app.modules.learning.items_repository.start_of_today_utc", return_value=NOW.replace(hour=0)
    ):
...
async def test_clock_is_sampled_after_waiting_for_owned_row_lock(practice_sql):
    from app.modules.learning import practice_repository as repo
    ...
    with (
        ...
        patch("app.modules.learning.practice.datetime", clock),
    ):
...
async def test_replay_heals_cache_after_committed_response_failure(practice_sql):
    ...
    with patch(
        "app.modules.learning.practice._invalidate_home_for_user", AsyncMock()
    ) as invalidate:
...
async def test_actual_sql_daily_items_keep_completed_reviews_on_original_day(practice_sql):
    from app.modules.learning.items_repository import (
