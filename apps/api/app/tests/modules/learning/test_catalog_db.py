...
from app.models.orm import Learning, LearningItem, QuizMissEvent, User
from app.modules.learning import catalog_repository as catalog_repo
from app.modules.learning import catalog_sync
...
async def _rows(session, monkeypatch):
    ...
    project = Learning(id=uuid4(), user_id=user.id, title="Spanish", target_language="es")
    ...
    item = (
        await session.execute(select(LearningItem).where(LearningItem.project_id == project.id))
    ).scalar_one()
...
async def test_catalog_write_is_idempotent_and_owner_scoped(db_session, monkeypatch):
    ...
    count = await db_session.scalar(
        select(func.count()).select_from(LearningItem).where(LearningItem.project_id == project.id)
    )
...
async def test_runtime_retirement_cascades_only_retired_owned_language_history(
    ...
    outside_project = Learning(
        id=uuid4(),
        user_id=user.id,
        title="French",
        target_language="fr",
    )
    outside_item = LearningItem(
        id=uuid4(),
        user_id=user.id,
    ...
    assert (
        await db_session.scalar(select(LearningItem.id).where(LearningItem.id == retired_id))
        == retired_id
    )
    await catalog_repo.delete_retired(
        db_session,
    ...
    assert (
        await db_session.scalar(select(LearningItem.id).where(LearningItem.id == outside_id))
        == outside_id
    )
        ...
        (
            await db_session.scalars(
                select(LearningItem.id).where(
                    LearningItem.id.in_([current_id, other_id, outside_id])
                )
            )
        ).all()
