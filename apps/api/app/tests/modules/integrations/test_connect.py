...
async def test_revoke_on_disconnect_calendar_revokes_shared_token():
    ...
    with (
        patch(
            "app.modules.integrations.connect.calendar_repo.get_for_user",
            AsyncMock(return_value=calendar),
        ),
        patch(
            "app.modules.integrations.connect.gmail_repo.get_for_user",
            AsyncMock(return_value=gmail),
        ),
        patch(
            "app.modules.integrations.connect.decrypt_refresh_token",
            side_effect=lambda _s, token: "shared-token" if token else "",
        ),
        patch(
            "app.modules.integrations.connect.google_oauth_revoke.revoke_refresh_token",
...
async def test_connect_calendar_rejects_when_feature_disabled():
    ...
    with (
        patch(
            "app.modules.integrations.connect.exchange_server_auth_code",
            AsyncMock(),
        ) as exchange_mock,
        patch(
            "app.modules.integrations.connect.calendar_repo.upsert",
            AsyncMock(),
        ) as upsert_mock,
    ):
...
async def test_connect_calendar_rejects_missing_calendar_readonly_scope():
    ...
    with (
        patch(
            "app.modules.integrations.connect.google_calendar_gateway.is_configured",
            return_value=True,
        ),
        patch(
            "app.modules.integrations.connect.exchange_server_auth_code",
        ...
        patch(
            "app.modules.integrations.connect._resolve_stored_refresh_token",
            return_value="enc-rt",
        ),
        patch(
            "app.modules.integrations.connect.google_oauth.fetch_google_email",
...
async def test_connect_calendar_accepts_calendar_readonly_scope():
    ...
    with (
        ...
        patch(
            "app.modules.integrations.connect.calendar_service.clear_events_cache",
            AsyncMock(),
        ),
        patch(
...
async def test_disconnect_calendar_deletes_row_when_revoke_decrypt_fails():
    ...
    with (
        ...
        patch(
            "app.modules.integrations.connect.calendar_repo.delete_for_user",
            AsyncMock(),
        ) as delete_mock,
        patch(
            "app.modules.integrations.connect.calendar_service.clear_events_cache",
...
async def test_disconnect_calendar_drops_gmail_when_token_was_shared():
    ...
    with (
        ...
        patch(
            "app.modules.integrations.connect.email_service.clear_gmail_cache",
            AsyncMock(),
        ),
        patch(
            "app.modules.integrations.connect.home_service.invalidate_home_cache",
...
async def test_connect_gmail_reuses_calendar_refresh_when_google_omits_token():
    ...
    with (
        patch(
            "app.modules.integrations.connect.exchange_gmail_auth_code",
            AsyncMock(return_value=token_data),
        ),
        patch(
            "app.modules.integrations.connect.gmail_repo.get_for_user",
        ...
        patch(
            "app.modules.integrations.connect.gmail_repo.upsert",
            AsyncMock(),
        ) as gmail_upsert,
        patch(
            "app.modules.integrations.connect.calendar_repo.upsert",
        ...
        patch(
            "app.modules.integrations.connect.jobs.enqueue",
            AsyncMock(),
        ),
        patch(
            "app.modules.integrations.connect.home_service.invalidate_home_cache",
