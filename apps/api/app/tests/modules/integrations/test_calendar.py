...
def test_format_calendar_block_skips_past_events():
    from app.modules.integrations.calendar import _events_within_days
...
def test_is_calendar_create_request(text, expected):
    from app.modules.integrations.calendar import is_calendar_create_request
...
def test_datetime_from_iso_parses_z_suffix():
    from app.modules.integrations.calendar import datetime_from_iso
...
async def test_is_connected():
    ...
    from app.modules.integrations.calendar import is_connected

    session = AsyncMock()
    with patch(
        "app.modules.integrations.calendar.calendar_repo.get_for_user",
...
async def test_has_write_access():
    ...
    from app.modules.integrations.calendar import has_write_access
...
async def test_list_events_for_api_reports_fetch_failure():
    ...
    from app.modules.integrations.calendar import list_events_for_api
    ...
    with (
        patch(
            "app.modules.integrations.calendar.is_connected",
        ...
        patch(
            "app.modules.integrations.calendar.google_calendar_gateway.list_upcoming_events",
...
@pytest.mark.asyncio
async def test_list_events_disconnects_on_permanent_oauth_error():
    from unittest.mock import AsyncMock, MagicMock, patch
    from uuid import uuid4
    ...
    with (
        ...
        patch(
            "app.modules.integrations.calendar.decrypt_refresh_token",
            return_value="refresh",
        ),
        patch(
            "app.modules.integrations.calendar.google_calendar_gateway.list_upcoming_events",
            AsyncMock(side_effect=GoogleCalendarError("expired", permanent=True)),
        ),
        patch(
            "app.modules.integrations.connect.disconnect_calendar",
            disconnect,
        ),
    ):
        result = await list_events_for_api(session, redis, user, settings)
...
async def test_fetch_upcoming_events_uses_redis_cache():
    ...
    from app.core.config import Settings
    from app.modules.integrations.calendar import fetch_upcoming_events
...
async def test_load_calendar_for_prompt_not_configured():
    ...
    from app.core.config import Settings
    from app.modules.integrations.calendar import load_calendar_for_prompt

    with patch(
        "app.modules.integrations.calendar.google_calendar_gateway.is_configured",
        return_value=False,
    ):
...
async def test_load_calendar_for_prompt_notes_partial_failure():
    ...
    with (
        ...
        patch(
            "app.modules.integrations.calendar._load_cached_events",
        ...
        patch(
            "app.modules.integrations.calendar._store_cached_events",
            AsyncMock(),
        ),
    ):
...
async def test_confirm_create_claims_proposal_before_google():
    ...
    from app.core.config import Settings
    from app.gateways.google_calendar_gateway import CalendarEvent
    from app.modules.integrations.calendar import confirm_create_event
    ...
    with (
        ...
        patch(
            "app.modules.integrations.calendar.load_event_proposal",
            AsyncMock(
                return_value={
                    "title": "Sync",
        ...
        patch(
            "app.modules.integrations.calendar.google_calendar_gateway.create_event",
            create,
        ),
    ):
...
async def test_store_and_load_event_proposal(fake_redis):
    ...
    from app.modules.integrations.calendar import load_event_proposal, store_event_proposal
