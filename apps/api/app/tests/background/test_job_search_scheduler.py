...
from app.background import job_search_jobs, job_search_scheduler
...
async def test_job_search_cycle_is_disabled_without_web_search() -> None:
    enqueue = AsyncMock()
    with (
        patch("app.background.job_search_scheduler.SessionLocal") as session_local,
        patch("app.background.job_search_scheduler.enqueue", enqueue),
    ):
...
async def test_job_search_cycle_enqueues_due_profiles_with_stable_dedupe_keys() -> None:
    ...
    with (
        patch(
            "app.background.job_search_scheduler.SessionLocal",
            return_value=_SessionCM(session),
        ),
        patch(
            "app.background.job_search_scheduler.get_redis_client",
            return_value=redis,
        ),
        patch("app.background.job_search_scheduler.enqueue", enqueue),
    ):
...
async def test_job_search_worker_forwards_scheduled_run_payload() -> None:
    ...
    run = AsyncMock()
    with (
        patch("app.background.job_search_jobs.get_redis_client", return_value=redis),
        patch("app.background.job_search_jobs.job_search_runner.run_job_search", run),
    ):
