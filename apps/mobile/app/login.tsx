    ...
    <View style={s.root}>
      ...
      <View style={s.sheet}>
        <Pressable
          ...
          {busy === 'google' ? <ActivityIndicator color="#333" /> : (
            <>
              <GoogleG />
              <Text style={s.googleText}>
                {isExpoGo() ? 'Google Sign-In (dev build only)' : 'Continue with Google'}
              </Text>
            </>
          )}
        </Pressable>
...
const g = StyleSheet.create({
  circle: { width: 24, height: 24, borderRadius: 12, backgroundColor: '#4285F4', alignItems: 'center', justifyContent: 'center', marginRight: 8 },
...
const s = StyleSheet.create({
  ...
  sp1: { position: 'absolute', top: -12, right: -16, fontSize: 16, color: C.primary, opacity: 0.7 },
  sp2: { position: 'absolute', bottom: 0, left: -18, fontSize: 22, color: C.primary, opacity: 0.4 },
  sp3: { position: 'absolute', bottom: -10, right: -6, fontSize: 12, color: C.primaryDark, opacity: 0.6 },
  title: { fontSize: 40, fontWeight: '800', color: '#1A0F4F', letterSpacing: -1 },
  subtitle: { fontSize: 17, color: '#5A4F7A', marginTop: 6 },
  sheet: {
    backgroundColor: C.bg, borderTopLeftRadius: 28, borderTopRightRadius: 28,
    paddingHorizontal: 24, paddingTop: 32, paddingBottom: 48, alignItems: 'center', gap: 12,
  },
  googleBtn: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    width: '100%', borderRadius: 14, borderWidth: 1.5, borderColor: C.border,
  ...
  devBtn: { width: '100%', borderRadius: 14, paddingVertical: 14, alignItems: 'center', backgroundColor: C.primaryLight },
