...
export default function MemoryScreen() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try { setMemories(await api.listMemories(token)); } finally { setLoading(false); }
  }, [token]);
  ...
  if (memories.length === 0) {
    return (
      <View style={s.empty}>
        <Ionicons name="sparkles-outline" size={48} color={C.primary} style={{ opacity: 0.5, marginBottom: 16 }} />
        <Text style={s.emptyTitle}>No memories yet</Text>
        <Text style={s.emptyBody}>
          Chat with Recall and it will automatically learn your preferences, projects, and facts about you.
        </Text>
      </View>
    );
  }
          ...
          {groups[type].map((m) => (
            <View key={m.id} style={s.card}>
              <View style={s.cardMain}>
                ...
                {m.confidence != null && (
                  <Text style={s.conf}>{Math.round(m.confidence * 100)}% confident</Text>
                )}
              </View>
              <Pressable
                hitSlop={8}
...
const s = StyleSheet.create({
  ...
  emptyBody: { fontSize: 15, color: C.textSecondary, marginTop: 6, textAlign: 'center' },
  root: { flex: 1, backgroundColor: C.bg },
  content: { padding: 16 },
  heading: { fontSize: 20, fontWeight: '700', color: C.text, marginBottom: 20 },
  group: { marginBottom: 24 },
  groupTitle: { fontSize: 13, fontWeight: '700', color: C.text, marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 },
  card: { flexDirection: 'row', alignItems: 'flex-start', backgroundColor: C.surface, borderRadius: 12, padding: 12, marginBottom: 8, gap: 8 },
  cardMain: { flex: 1 },
