...
import { fetchMemories, getCachedMemories, updateMemoriesCache } from "@/features/memory/model/memoryListCache";
...
export function useMemoryActions(token: string | null) {
  ...
  const deleteFact = useCallback(async (section: Memory): Promise<boolean> => {
    if (!token || !isCurrentOwner()) return false;
    const snapshot = getCachedMemories()?.find((row) => row.id === section.id);
    if (!snapshot) return false;
    const remove: MemoryUpdate = (rows) => rows.filter((row) => row.id !== snapshot.id);
    return mutate(snapshot.type, remove, (rows) => restoreRows(rows, [snapshot]), async () => {
      await api.deleteMemory(token, snapshot.id);
      return remove;
    });
  }, [token, isCurrentOwner, mutate]);
