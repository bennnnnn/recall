...
describe("scanner subjects", () => {
  it("maps each subject to a distinct protocol prompt", () => {
    ...
    expect(scannerCameraPrompt("chemistry")).toBe(CHEMISTRY_CAMERA_PROMPT);
  });
  ...
  it("recognizes all scanner prompts without claiming normal image questions", () => {
    ...
    expect(isScannerCameraPrompt(CHEMISTRY_CAMERA_PROMPT.toUpperCase())).toBe(true);
  ...
  it("always carries the selected subject while preserving a typed caption", () => {
    ...
    expect(composerTextAfterSubjectScan("my own caption", "chemistry")).toBe(
      `${CHEMISTRY_CAMERA_PROMPT}\n\nmy own caption`,
    );
  });
});
